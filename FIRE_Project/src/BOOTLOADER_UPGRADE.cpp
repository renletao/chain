...
uint32_t BOOTLOADER_UPGRADE::compute_cfg_crc32(const unsigned char *cfg, uint16_t len)
{
    uint32_t chk_sum = 0xFFFFFFFF;
    uint8_t *p_data = (uint8_t*)cfg;
...
void BOOTLOADER_UPGRADE::func_iap_update(void)
{
    int8_t error = 1;
    uint8_t sendbuffer[2056];
    uint16_t loop=0;
    uint16_t buffer_index = 0;//*endp; 

    uint32_t APPLICATION_ADDRESS = 0x08001800;
    ...
    sendbuffer[0] = OPC_WREN;
    _wire->beginTransmission(BOOTLOADER_UPGRADE_ADDR);
    _wire->write(sendbuffer, 1);
    _wire->endTransmission(); 
}
...
void BOOTLOADER_UPGRADE::func_iap_upgrade(const unsigned char *app_buffer)
{
    ...
    do {
        ...
        if(sendbuffer_Size >= 2048) {
            ...
            _wire->beginTransmission(BOOTLOADER_UPGRADE_ADDR);
            _wire->write(sendbuffer, 2056);
            _wire->endTransmission(); 
            sendbuffer_Size -= 2048;
            APPLICATION_ADDRESS += 2048;
            process_status++;
        //*temp +=128;
        } else {
            _wire->beginTransmission(BOOTLOADER_UPGRADE_ADDR); 
...
void BOOTLOADER_UPGRADE::jumpBootloader(void) {
    ...
    temp[2] = 0x55;
    temp[3] = 0xAA;
    writeBytes(_addr, DMM_SCRATCH_PAD_REG1, (uint8_t *)&temp[0], 4);
    temp[0] = CMD_JMP_BOOTLOADER;
    writeBytes(_addr, DMM_EXE_CMD_REG, (uint8_t *)&temp[0], 1);
}
