...
VL53L1X::VL53L1X()
    ...
    : bus(nullptr)
      ...
      ,
      did_timeout(false), calibrated(false), saved_vhv_init(0),
...
bool VL53L1X::init(bool io_2v8) {
  ...
  writeReg16Bit(DSS_CONFIG__TARGET_TOTAL_RATE_MCPS,
                TargetRate); // should already be this value after reset
  writeReg(GPIO__TIO_HV_STATUS, 0x02);
  writeReg(SIGMA_ESTIMATOR__EFFECTIVE_PULSE_WIDTH_NS, 8); // tuning parm default
  writeReg(SIGMA_ESTIMATOR__EFFECTIVE_AMBIENT_WIDTH_NS,
  ...
  writeReg16Bit(RANGE_CONFIG__MIN_COUNT_RATE_RTN_LIMIT_MCPS,
  ...
  writeReg32Bit(SYSTEM__INTERMEASUREMENT_PERIOD, 50 * osc_calibrate_val);
...
bool VL53L1X::setDistanceMode(uint8_t mode) {
  switch (mode) {
  case 0:
...
bool VL53L1X::setMeasurementTimingBudget(uint32_t budget_us) {
  ...
  writeReg16Bit(RANGE_CONFIG__TIMEOUT_MACROP_A,
                encodeTimeout(timeoutMicrosecondsToMclks(
...
uint32_t VL53L1X::getMeasurementTimingBudget() {
  ...
  // "Update Macro Period for Range A VCSEL Period"
  uint32_t macro_period_us =
      calcMacroPeriod(readReg(RANGE_CONFIG__VCSEL_PERIOD_A));

  // "Get Range Timing A timeout"

  uint32_t range_config_timeout_us = timeoutMclksToMicroseconds(
      decodeTimeout(readReg16Bit(RANGE_CONFIG__TIMEOUT_MACROP_A)),
      macro_period_us);
...
void VL53L1X::setupManualCalibration() {
  ...
  writeReg(
      ...
      (saved_vhv_timeout & 0x03) +
          (3
           << 2)); // tuning parm default (LOWPOWERAUTO_VHV_LOOP_BOUND_DEFAULT)
...
void VL53L1X::readResults() {
  ...
  results.dss_actual_effective_spads_sd0 = (uint16_t)bus->read()
                                           << 8;         // high byte
  ...
  results.final_crosstalk_corrected_range_mm_sd0 = (uint16_t)bus->read()
                                                   << 8;         // high byte
...
uint32_t VL53L1X::timeoutMclksToMicroseconds(uint32_t timeout_mclks,
                                             uint32_t macro_period_us) {
...
uint32_t VL53L1X::timeoutMicrosecondsToMclks(uint32_t timeout_us,
                                             uint32_t macro_period_us) {
