...
extern __IO uint8_t g_uart_in_buf_status;
extern __IO uint8_t g_uart_out_buf_status;
...
/* USER CODE BEGIN 0 */
/**
 * @brief Get the UART input data packet type based on the contents of the
 * buffer.
 * @note This function checks the data packet in the buffer to determine its
 * type.
 *       It verifies the packet integrity using the `pack_check` function, and
 * then
...
/**
 * @brief Get the UART output data packet type based on the contents of the
 * buffer.
 * @note This function checks the data packet in the buffer to determine its
 * type.
 *       It verifies the packet integrity using the `pack_check` function, and
 * then
 *       inspects specific bytes in the buffer to classify the packet type for
 * output.
 *
