...
void VL53L0X_writeReg(VL53L0X* dev, uint8_t reg, uint8_t value)
{
	...
	dev->last_status= HAL_I2C_Master_Transmit(&hi2c2, dev->address, buf, 2, 0XFFFF);
}
...
void VL53L0X_writeReg16Bit( VL53L0X* dev, uint8_t reg, uint16_t value)
{
	...
	dev->last_status= HAL_I2C_Master_Transmit(&hi2c2, dev->address, buf, 3, 0XFFFF);
}
...
void VL53L0X_writeReg32Bit( VL53L0X* dev, uint8_t reg, uint32_t value)
{
	...
	dev->last_status=HAL_I2C_Master_Transmit(&hi2c2, dev->address, buf, 5, 0XFFFF);
}
...
uint8_t VL53L0X_readReg(VL53L0X* dev, uint8_t reg)
{
  ...
  HAL_I2C_Master_Transmit(&hi2c2, dev->address, &reg, 1, 0XFFFF);
  //dev->last_status = i2c_read(dev->address, &value, 1);
  dev->last_status=HAL_I2C_Master_Receive(&hi2c2, dev->address, &value, 1, 0XFFFF);
  return value;
}
...
uint16_t VL53L0X_readReg16Bit(VL53L0X* dev, uint8_t reg)
{
  ...
  dev->last_status=HAL_I2C_Master_Receive(&hi2c2, dev->address, buf, 2, 0XFFFF);
...
uint32_t VL53L0X_readReg32Bit( VL53L0X* dev, uint8_t reg)
{
  ...
  dev->last_status=HAL_I2C_Master_Receive(&hi2c2, dev->address, buf, 4, 0XFFFF);
...
void VL53L0X_writeMulti(VL53L0X* dev, uint8_t reg, uint8_t* src, uint8_t count)
{
	//i2c_write(dev->address, &reg, 1);
	HAL_I2C_Master_Transmit(&hi2c2, dev->address, &reg, 1, 0xFFFF);
	dev->last_status =HAL_I2C_Master_Transmit(&hi2c2, dev->address, src, count, 0XFFFF);
}
...
void VL53L0X_readMulti(VL53L0X* dev, uint8_t reg, uint8_t * dst, uint8_t count)
{
	//i2c_write(dev->address, &reg, 1);
	HAL_I2C_Master_Transmit(&hi2c2, dev->address, &reg, 1, 0xFFFF);
	dev->last_status =HAL_I2C_Master_Receive(&hi2c2, dev->address, dst, count, 0XFFFF);
}
