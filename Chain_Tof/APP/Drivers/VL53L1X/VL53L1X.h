...
#define VL53L1X_ADDR (0x29)
	...
	typedef struct
	{
		uint8_t range_status; // Range status
		// uint8_t report_status: not used                  // Report status: not used
		uint8_t stream_count;					 // Stream count
		uint16_t dss_actual_effective_spads_sd0; // Actual effective SPAD count for SD0
		// uint16_t peak_signal_count_rate_mcps_sd0: not used // Peak signal count rate (SD0): not used
		uint16_t ambient_count_rate_mcps_sd0; // Ambient count rate (SD0), in mega counts per second
		// uint16_t sigma_sd0: not used                      // Sigma value (SD0): not used
		// uint16_t phase_sd0: not used                      // Phase value (SD0): not used
		uint16_t final_crosstalk_corrected_range_mm_sd0;			  // Final crosstalk corrected range, in millimeters
		uint16_t peak_signal_count_rate_crosstalk_corrected_mcps_sd0; // Crosstalk corrected peak signal count rate, in mega counts per second
	} ResultBuffer;

	typedef enum
	{
		RangeValid = 0,				   // Range is valid
		SigmaFail = 1,				   // Sigma failure
		SignalFail = 2,				   // Signal failure
		RangeValidMinRangeClipped = 3, // Valid range, minimum range clipped
		OutOfBoundsFail = 4,		   // Out of bounds failure
		HardwareFail = 5,			   // Hardware failure
		RangeValidNoWrapCheckFail = 6, // Valid range, no wrap check failure
		WrapTargetFail = 7,			   // Wrap target failure
		XtalkSignalFail = 9,		   // Crosstalk signal failure
		SynchronizationInt = 10,	   // Synchronization interrupt
		MinRangeFail = 13,			   // Minimum range failure
		None = 255,					   // No status
	} RangeStatus;
	...
	typedef struct
	{
		...
		RangeStatus range_status;		   // Range status
		float peak_signal_count_rate_MCPS; // Peak signal count rate, in mega counts per second
		float ambient_count_rate_MCPS;	   // Ambient count rate, in mega counts per second
	} RangingData;

	bool vl53l1xInit(uint8_t mode); // Initialize
	uint16_t getDistance(void);		// Get distance
